    ...
    <section className="relative overflow-hidden border-b border-stone-200 bg-stone-900 text-white">
      ...
      <div className="relative mx-auto max-w-7xl px-4 py-12 lg:px-8 lg:py-16">
        <nav className="mb-4 flex items-center gap-1.5 text-sm text-stone-300">
          ...
          <span className="text-white">{messages.common.events}</span>
        </nav>
        <h1 className="font-display text-3xl font-bold md:text-4xl">{messages.events.title}</h1>
        <p className="mt-2 max-w-2xl text-sm text-stone-300 md:text-base">
          {messages.events.intro}
        </p>
        ...
        >
          <Search className="ml-2 h-5 w-5 flex-shrink-0 text-white/70" />
          <input
            value={searchInput}
            onChange={(e) => onSearchInputChange(e.target.value)}
            placeholder={messages.events.searchPlaceholder}
            className="min-w-0 flex-1 bg-transparent text-white placeholder:text-white/60 outline-none"
          />
          {searchInput && (
            <button
              type="button"
              ...
              aria-label={messages.events.clearSearch}
          ...
          <button
            type="submit"
            className="rounded-xl bg-amber-500 px-4 py-2.5 text-sm font-semibold text-white shadow-soft transition-all hover:-translate-y-0.5 hover:bg-amber-600 sm:px-5"
          >
