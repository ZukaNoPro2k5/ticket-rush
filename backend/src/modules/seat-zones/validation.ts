...
export const createSeatZoneSchema = z.object({
  ...
  // Cross-field limits depend on event seating_mode and are enforced in service.ts.
  // For seated mode: rows A-Z (1-26), cols 1-50.
...
export const updateSeatZoneSchema = z.object({
  ...
  total_rows: z.coerce.number().int().min(1).max(26, 'Maximum 26 rows (A-Z)').optional(),
  total_cols: z.coerce.number().int().min(1).max(99999).optional(),
